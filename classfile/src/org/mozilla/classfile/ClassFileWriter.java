...
public class ClassFileWriter implements ByteCode {
    ...
    public ClassFileWriter(String className, String superClassName, String sourceFileName) {
        ...
        itsFlags = ACC_PUBLIC;
        if (DEBUGCODE)
            setDebugCodeOutput(System.out);
        if (isDebugCode())
            debugString(".class " + className + " extends " + superClassName);
    }
    ...
    public void addField(String fieldName, String type, short flags, double value) {
        ...
        itsFields.add(field);
        if (isDebugCode())
            debugString(".field " + modifierStr(flags) + fieldName + " " + type);
    }
     ...
     *            or -1 if it does not have a Java register.
     * @param stopPC
     *            the last bytecode PC where this variable is live,
     *            or -1 if the variable is live to the end of the method.
     */
    public void addVariableDescriptor(String name, String type, int startPC, int slot, int stopPC) {
        int nameIndex = itsConstantPool.addUtf8(name);
        int descriptorIndex = itsConstantPool.addUtf8(type);
        int[] chunk = { nameIndex, descriptorIndex, startPC, slot, stopPC };
        if (itsVarDescriptors == null) {
            itsVarDescriptors = new Vector<Object>();
    ...
    public void startMethod(String methodName, String type, short flags) {
        ...
        if (isDebugCode())
            debugString(".method " + modifierStr(flags) + methodName + type);
    }
    ...
    public void stopMethod(short maxLocals) {
        ...
        if (itsVarDescriptors != null) {
            ...
            for (int i = 0; i < varCount; i++) {
                ...
                int slot = chunk[3];
                int stopPC = chunk[4];
                int length = (stopPC == -1) ? (itsCodeBufferTop - startPC) : (stopPC - startPC);
                ...
                index = putInt16(slot, codeAttribute, index);
            }
        }
    ...
    public void add(int theOpCode, int theOperand1, int theOperand2) {
        if (isDebugCode()) {
            debugCode(bytecodeStr(theOpCode) + ", " + Integer.toHexString(theOperand1) + ", "
        ...
        if (theOpCode == ByteCode.IINC) {
            if (!(0 <= theOperand1 && theOperand1 < 65536))
                throw new ClassFileFormatException("out of range variable");
            if (!(-32768 <= theOperand2 && theOperand2 < 32767))
    ...
    public void add(int theOpCode, String className, int theOperand1) {
        ...
        if (theOpCode == ByteCode.MULTIANEWARRAY) {
            if (!(0 <= theOperand1 && theOperand1 < 256))
                throw new IllegalArgumentException("out of range dimensions");
    ...
    /**
     * Generate code to load the given float on stack.
     *
     * @param k
     *            the constant
     */
    public void addPush(float k) {
        if (k == 0.0) {
            // zero
            add(ByteCode.FCONST_0);
            if (1.0 / k < 0) {
                // Negative zero
                add(ByteCode.FNEG);
            }
        } else if (k == 1.0 || k == -1.0) {
            add(ByteCode.FCONST_1);
            if (k < 0) {
                add(ByteCode.FNEG);
            }
        } else {
    ...
    public int addLookupSwitch(int entryCount) {
        ...
        int N = addReservedCodeSpace(1 + padSize + 4 * (1 + 1 + entryCount * 2));
        int switchStart = N;
        ...
        N = putInt32(entryCount, itsCodeBuffer, N);

        itsStackTop = (short) newStack;
        if (newStack > itsMaxStack)
            itsMaxStack = (short) newStack;
        if (DEBUGSTACK) {
            System.out.println("After " + bytecodeStr(ByteCode.LOOKUPSWITCH) + " stack = " + itsStackTop);
        }

        return switchStart;
    }

    public final void addLookupSwitchDefaultLabel(int switchStart, int jumpTarget) {
        addLookupSwitchCaseLabel(switchStart, -1, -1, jumpTarget);
    }

    public void addLookupSwitchCaseLabel(int switchStart, int caseIndex, int caseKey, int jumpLabel) {
        if (isDebugCode())
            if (caseIndex < 0)
                debugString("\tdefault: #" + Integer.toString(jumpLabel & 0x7FFFFFFF));
            else
                debugString("\tcase: " + Integer.toString(caseKey) + ", #" + Integer.toString(jumpLabel & 0x7FFFFFFF));
        int jumpTarget = getLabelPC(jumpLabel);
        if (jumpTarget == 0)
            throw new IllegalStateException("Label must be marked before adding to LookupSwitch");
        setLookupSwitchCase(switchStart, caseIndex, caseKey, jumpTarget);
    }

    protected void setLookupSwitchCase(int switchStart, int caseIndex, int caseKey, int jumpTarget) {
        ...
        int defaultOffset = switchStart + 1 + padSize;
        int sizeOffset = defaultOffset + 4;
        int tableOffset = sizeOffset + 4;
        int caseOffset;
        if (caseIndex < 0) {
            // default label
            caseOffset = defaultOffset;
        } else {
            caseOffset = tableOffset + 4 * caseIndex * 2;
        }
        if (!(0 <= switchStart && switchStart <= itsCodeBufferTop - 2 * 4 - padSize - 1)) {
            throw new IllegalArgumentException(switchStart + " is outside a possible range of tableswitch"
                    + " in already generated code");
        }
        if (caseOffset > itsCodeBufferTop) {
            throw new IllegalArgumentException("LookupSwitch index is out of range: " + caseIndex);
        }
        if ((0xFF & itsCodeBuffer[switchStart]) != ByteCode.LOOKUPSWITCH) {
            throw new IllegalArgumentException(switchStart + " is not offset of lookupswitch statement");
        }
        ...
        // Insert (key, label) into lookupswitch table.
        if (caseIndex < 0) {
            // Default offset
            putInt32(jumpTarget - switchStart, itsCodeBuffer, caseOffset);
        } else {
            putInt32(caseKey, itsCodeBuffer, caseOffset);
    ...
    public final void addTableSwitchDefaultLabel(int switchStart, int caseLabel) {
        if (caseLabel < 0)
            addTableSwitchCaseLabel(switchStart, -1, caseLabel);
    }

    public final void addTableSwitchCaseLabel(int switchStart, int caseIndex, int caseLabel) {
        if (isDebugCode())
            if (caseIndex < 0)
                debugString("\tdefault: #" + Integer.toString(caseLabel & 0x7FFFFFFF));
            else
                debugString("\tcase: " + Integer.toString(caseIndex) + ", #" + Integer.toString(caseLabel & 0x7FFFFFFF));
    ...
    public void addExceptionHandler(int startLabel, int endLabel, int handlerLabel, String catchClassName) {
        ...
        if (isDebugCode())
            this.debugString("\t.catch " + (catchClassName == null ? "all" : catchClassName) + " from #"
                    + (startLabel & 0x7FFFFFFF) + " to #" + (endLabel & 0x7FFFFFFF) + " using #"
                    + (handlerLabel & 0x7FFFFFFF));
    ...
    public void addLineNumberEntry(short lineNumber) {
        ...
        int pc = itsCodeBufferTop;
        itsLineNumberTable[N] = (pc << 16) + lineNumber;
    ...
    */
     ...
     */
    public static int extra(int opcode) {
        switch (opcode) {
        case ByteCode.AALOAD:
        case ByteCode.AASTORE:
        case ByteCode.ACONST_NULL:
    ...
    public byte[] getCodeBuffer() {
        byte[] tmp = new byte[itsCodeBufferTop];
        System.arraycopy(itsCodeBuffer, 0, tmp, 0, itsCodeBufferTop);
        return tmp;
    }

    public byte[] getCodeAttribute() {
        ClassFileMethod method = (ClassFileMethod) itsMethods.lastElement();
        byte[] temp = method.getCodeAttribute();
        int size = temp.length - 14;
        byte[] data = new byte[size];
        System.arraycopy(temp, 14, data, 0, size);
    ...
    private void debugString(String message) {
        getDebugCodeOutput().println(message);
    }
    ...
    private Vector<Object> itsMethods = new Vector<Object>();
    private Vector<Object> itsFields = new Vector<Object>();
    private Vector<Object> itsInterfaces = new Vector<Object>();
    ...
    private Vector<Object> itsVarDescriptors;
...
final class ClassFileMethod {
    private static final byte[] EMPTY_CODE_ATTRIBUTE = new byte[0];

    ClassFileMethod(short nameIndex, short typeIndex, short flags) {
        itsNameIndex = nameIndex;
        itsTypeIndex = typeIndex;
        itsFlags = flags;
        itsCodeAttribute = EMPTY_CODE_ATTRIBUTE;
    }
    ...
    byte[] getCodeAttribute() {
        return itsCodeAttribute;
    }
...
final class ConstantPool {
    ...
    private int getStringConstHash(int utf8Index) {
        Integer theValue = itsStringConstHash.get(Integer.valueOf(utf8Index));
        return (theValue != null) ? theValue.intValue() : -1;
    }

    private void putStringConstHash(int utf8Index, int theIndex) {
        itsStringConstHash.put(Integer.valueOf(utf8Index), Integer.valueOf(theIndex));
    }

    private int getInt(HashMap<Object, Integer> map, Object key) {
        Integer theValue = map.get(key);
        return (theValue != null) ? theValue.intValue() : -1;
    }

    private void putInt(HashMap<Object, Integer> map, Object key, int theIndex) {
        map.put(key, Integer.valueOf(theIndex));
    }

    int addConstant(String k) {
        int utf8Index = 0xFFFF & addUtf8(k);
        //int theIndex = itsStringConstHash.getInt(utf8Index, -1);
        int theIndex = getStringConstHash(utf8Index);
        if (theIndex == -1) {
            theIndex = itsTopIndex++;
            ensure(3);
            itsPool[itsTop++] = CONSTANT_String;
            itsTop = ClassFileWriter.putInt16(utf8Index, itsPool, itsTop);
            putStringConstHash(utf8Index, theIndex);
        }
        return theIndex;
    }
    ...
    short addUtf8(String k) {
        int theIndex = getInt(itsUtf8Hash, k);
        if (theIndex == -1) {
            int strLen = k.length();
    ...
    short addClass(String className) {
        int theIndex = getInt(itsClassHash, className);
        if (theIndex == -1) {
            String slashed = className;
            if (className.indexOf('.') > 0) {
                slashed = ClassFileWriter.getSlashedForm(className);
                theIndex = getInt(itsClassHash, slashed);
                if (theIndex != -1) {
                    putInt(itsClassHash, className, theIndex);
                }
            }
            if (theIndex == -1) {
                ...
                itsTop = ClassFileWriter.putInt16(utf8Index, itsPool, itsTop);
                theIndex = itsTopIndex++;
                putInt(itsClassHash, slashed, theIndex);
    ...
    short addFieldRef(String className, String fieldName, String fieldType) {
        ...
        int theIndex = getInt(itsFieldRefHash, ref);
        if (theIndex == -1) {
            ...
            theIndex = itsTopIndex++;
            putInt(itsFieldRefHash, ref, theIndex);
        }
        return (short) theIndex;
    }
    ...
    short addMethodRef(String className, String methodName, String methodType) {
        ...
        int theIndex = getInt(itsMethodRefHash, ref);
        if (theIndex == -1) {
            ...
            itsTop = ClassFileWriter.putInt16(ntIndex, itsPool, itsTop);
            theIndex = itsTopIndex++;
            putInt(itsMethodRefHash, ref, theIndex);
        }
        return (short) theIndex;
    }
